...
class Solver:
    ...
    def _solve_cvrp(
        ...
        payload = _core.solve_cvrp(
            ...
            is_rounding_integer=is_rounding_integer,
            is_duration_constraint=is_duration_constraint,
            max_nb_veh=maximum_number_of_vehicles,
            nb_granular=algorithm_parameters.nbGranular,
            mu=algorithm_parameters.mu,
            lambda_value=algorithm_parameters.lambda_,
            nb_elite=algorithm_parameters.nbElite,
            nb_close=algorithm_parameters.nbClose,
            nb_iter_penalty_management=algorithm_parameters.nbIterPenaltyManagement,
            target_feasible=algorithm_parameters.targetFeasible,
            penalty_decrease=algorithm_parameters.penaltyDecrease,
            penalty_increase=algorithm_parameters.penaltyIncrease,
            seed=algorithm_parameters.seed,
            nb_iter=algorithm_parameters.nbIter,
            nb_iter_traces=algorithm_parameters.nbIterTraces,
            time_limit=algorithm_parameters.timeLimit,
            use_swap_star=algorithm_parameters.useSwapStar,
            verbose=verbose,
        )
    ...
    def _solve_cvrp_dist_mtx(
        ...
        payload = _core.solve_cvrp_dist_mtx(
            ...
            x=np.ascontiguousarray(x_coords, dtype=np.float64),
            y=np.ascontiguousarray(y_coords, dtype=np.float64),
            dist_mtx=np.ascontiguousarray(dist_mtx.reshape(n_nodes * n_nodes), dtype=np.float64),
            service_times=np.ascontiguousarray(service_times, dtype=np.float64),
            demand=np.ascontiguousarray(demand, dtype=np.float64),
            vehicle_capacity=vehicle_capacity,
            duration_limit=duration_limit,
